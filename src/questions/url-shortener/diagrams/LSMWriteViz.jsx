  ...
  return (
    <div className="bg-white rounded-xl border border-ink-200 overflow-hidden my-8 not-prose">
      ...
      <div className="p-6 space-y-4">
        {/* Incoming writes */}
        <div className="border border-ink-200 rounded-lg bg-cream-50 p-4">
          <div className="text-xs text-ink-500 uppercase tracking-wider mb-3">1. Client writes arriving</div>
            ...
            <AnimatePresence>
              {writes.map((w) => (
                <motion.div
                  ...
                  className="bg-white border border-ink-300 font-mono text-xs px-2 py-1 rounded"
                >
                  PUT {w.key}
                </motion.div>
              ))}
        ...
        {/* Step 2 - WAL */}
        <div className="border border-rust-200 bg-rust-50 rounded-lg p-4">
          <div className="flex items-baseline justify-between mb-2">
            <div className="text-xs text-rust-600 uppercase tracking-wider font-medium">2. Append to WAL (disk)</div>
            <div className="text-[10px] text-ink-500 font-mono">~microseconds</div>
          </div>
        ...
        <div className="border border-teal-200 bg-teal-50 rounded-lg p-4">
          <div className="flex items-baseline justify-between mb-2">
            <div className="text-xs text-teal-700 uppercase tracking-wider font-medium">3. Insert into memtable (RAM)</div>
            <div className="text-[10px] text-ink-500 font-mono">in-memory sorted map</div>
          </div>
            ...
            <AnimatePresence>
              {writes.map((w) => (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="bg-white border border-teal-300 font-mono text-[10px] px-2 py-1 rounded text-center truncate"
                >
                  {w.key}
                </motion.div>
              ))}
        ...
        {/* Step 4 - Flush */}
        <div className="border border-ink-200 bg-white rounded-lg p-4 opacity-70">
          <div className="text-xs text-ink-500 uppercase tracking-wider mb-2">4. Background flush to SSTable</div>
